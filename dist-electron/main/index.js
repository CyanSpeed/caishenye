...
function initDatabase() {
  const dbPath = path.join(electron.app.getPath("userData"), "finance.db");
...
function getAllAccounts() {
  ...
  const rows = db2.prepare("SELECT * FROM accounts ORDER BY id").all();
...
function updateAccount(id, updates) {
  ...
  values.push(id);
  db2.prepare(`UPDATE accounts SET ${fields.join(", ")} WHERE id = ?`).run(...values);
}
function deleteAccount(id) {
  const db2 = getDatabase();
  db2.prepare("DELETE FROM accounts WHERE id = ?").run(id);
}
function getAllCategories() {
  const db2 = getDatabase();
  return db2.prepare("SELECT * FROM categories ORDER BY id").all();
}
function getAllTransactions() {
  const db2 = getDatabase();
  return db2.prepare("SELECT * FROM transactions ORDER BY date DESC, id DESC").all();
}
function addTransaction(tx) {
  const db2 = getDatabase();
  const insertAndBalance = db2.transaction(() => {
    const result = db2.prepare(
    ...
    if (tx.type === "expense" && tx.from_account_id) {
      const account = db2.prepare("SELECT balance FROM accounts WHERE id = ?").get(tx.from_account_id);
      if (account) {
        const newBalance = new Decimal(account.balance).minus(amount).toFixed(2);
        db2.prepare("UPDATE accounts SET balance = ? WHERE id = ?").run(newBalance, tx.from_account_id);
      }
    } else if (tx.type === "income" && tx.to_account_id) {
      const account = db2.prepare("SELECT balance FROM accounts WHERE id = ?").get(tx.to_account_id);
      if (account) {
        const newBalance = new Decimal(account.balance).plus(amount).toFixed(2);
        db2.prepare("UPDATE accounts SET balance = ? WHERE id = ?").run(newBalance, tx.to_account_id);
      }
    } else if (tx.type === "transfer") {
      if (tx.from_account_id) {
        const from = db2.prepare("SELECT balance FROM accounts WHERE id = ?").get(tx.from_account_id);
        if (from) {
          db2.prepare("UPDATE accounts SET balance = ? WHERE id = ?").run(new Decimal(from.balance).minus(amount).toFixed(2), tx.from_account_id);
        }
      }
      if (tx.to_account_id) {
        const to = db2.prepare("SELECT balance FROM accounts WHERE id = ?").get(tx.to_account_id);
        if (to) {
          db2.prepare("UPDATE accounts SET balance = ? WHERE id = ?").run(new Decimal(to.balance).plus(amount).toFixed(2), tx.to_account_id);
        }
      }
    }
...
function getAllInvestmentSnapshots() {
  const db2 = getDatabase();
  return db2.prepare("SELECT * FROM investment_snapshots ORDER BY date DESC").all();
}
function addInvestmentSnapshot(snapshot) {
  const db2 = getDatabase();
  const result = db2.prepare(
...
function updateInvestmentSnapshot(id, updates) {
  const db2 = getDatabase();
  const fields = [];
  const values = [];
  if (updates.account_id !== void 0) {
  ...
  if (updates.market_value !== void 0) {
    fields.push("market_value = ?");
    values.push(updates.market_value);
  }
  if (updates.cost_basis !== void 0) {
    fields.push("cost_basis = ?");
    values.push(updates.cost_basis);
  }
  ...
  if (fields.length === 0) return;
  values.push(id);
  db2.prepare(`UPDATE investment_snapshots SET ${fields.join(", ")} WHERE id = ?`).run(...values);
}
function getAllPhysicalAssets() {
  const db2 = getDatabase();
  return db2.prepare("SELECT * FROM physical_assets ORDER BY id").all();
}
function addPhysicalAsset(asset) {
  const db2 = getDatabase();
  const result = db2.prepare(
...
function updatePhysicalAsset(id, updates) {
  const db2 = getDatabase();
  const fields = [];
  const values = [];
  if (updates.name !== void 0) {
  ...
  if (updates.icon_emoji !== void 0) {
    fields.push("icon_emoji = ?");
    values.push(updates.icon_emoji);
  }
  if (updates.purchase_price !== void 0) {
    fields.push("purchase_price = ?");
    values.push(updates.purchase_price);
  }
  if (updates.purchase_date !== void 0) {
    fields.push("purchase_date = ?");
    values.push(updates.purchase_date);
  }
  if (updates.current_value !== void 0) {
    fields.push("current_value = ?");
    values.push(updates.current_value);
  }
  if (updates.image_url !== void 0) {
  ...
  if (fields.length === 0) return;
  values.push(id);
  db2.prepare(`UPDATE physical_assets SET ${fields.join(", ")} WHERE id = ?`).run(...values);
}
function deletePhysicalAsset(id) {
  const db2 = getDatabase();
  db2.prepare("DELETE FROM physical_assets WHERE id = ?").run(id);
}
const IPC_CHANNELS = {
  PING: "ping",
  // Accounts
...
function registerIpcHandlers() {
  electron.ipcMain.handle(IPC_CHANNELS.PING, () => "pong");
  electron.ipcMain.handle(IPC_CHANNELS.GET_ACCOUNTS, () => getAllAccounts());
  electron.ipcMain.handle(IPC_CHANNELS.ADD_ACCOUNT, (_event, account) => addAccount(account));
  electron.ipcMain.handle(IPC_CHANNELS.UPDATE_ACCOUNT, (_event, id, updates) => updateAccount(id, updates));
  electron.ipcMain.handle(IPC_CHANNELS.DELETE_ACCOUNT, (_event, id) => deleteAccount(id));
  electron.ipcMain.handle(IPC_CHANNELS.GET_CATEGORIES, () => getAllCategories());
  electron.ipcMain.handle(IPC_CHANNELS.GET_TRANSACTIONS, () => getAllTransactions());
  electron.ipcMain.handle(IPC_CHANNELS.ADD_TRANSACTION, (_event, tx) => addTransaction(tx));
  electron.ipcMain.handle(IPC_CHANNELS.GET_INVESTMENT_SNAPSHOTS, () => getAllInvestmentSnapshots());
  electron.ipcMain.handle(IPC_CHANNELS.ADD_INVESTMENT_SNAPSHOT, (_event, snapshot) => addInvestmentSnapshot(snapshot));
  electron.ipcMain.handle(IPC_CHANNELS.UPDATE_INVESTMENT_SNAPSHOT, (_event, id, updates) => updateInvestmentSnapshot(id, updates));
  electron.ipcMain.handle(IPC_CHANNELS.GET_PHYSICAL_ASSETS, () => getAllPhysicalAssets());
  electron.ipcMain.handle(IPC_CHANNELS.ADD_PHYSICAL_ASSET, (_event, asset) => addPhysicalAsset(asset));
  electron.ipcMain.handle(IPC_CHANNELS.UPDATE_PHYSICAL_ASSET, (_event, id, updates) => updatePhysicalAsset(id, updates));
  electron.ipcMain.handle(IPC_CHANNELS.DELETE_PHYSICAL_ASSET, (_event, id) => deletePhysicalAsset(id));
}
